...
class DataFeeder(threading.Thread):
  ...
  def __init__(self, coordinator, metadata_filename, hparams):
    ...
    with open(metadata_filename, encoding='utf-8') as f:
      ...
      tf.logging.info('Loaded metadata for %d examples (%.2f hours)' % (len(self._metadata), hours))
    ...
    if hparams.use_cmudict:
      ...
      tf.logging.info('Loaded CMUDict with %d unambiguous entries' % len(self._cmudict))
    else:
      self._cmudict = None
  ...
  def _enqueue_next_group(self):
    ...
    tf.logging.info('Generated %d batches of size %d in %.03f sec' % (len(batches), n, time.time() - start))
